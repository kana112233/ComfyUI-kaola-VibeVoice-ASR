...
class VibeVoiceForConditionalGenerationInference(VibeVoicePreTrainedModel, GenerationMixin):
    ...
    def tie_weights(self, **kwargs):
        ...
        """
        # Always tie weights - the checkpoint has tie_word_embeddings=true in config.json
        # but VibeVoiceConfig may not expose this attribute correctly
        if hasattr(self, 'lm_head') and hasattr(self.model, 'language_model') and hasattr(self.model.language_model, 'embed_tokens'):
    ...
    def generate(
       ...
       # Define all valid tokens that can be generated
        ...
        # TEMPORARILY DISABLED: The constraint processor causes NaN when model assigns low prob to speech tokens
        ...
        for step in progress_bar:
            ...
            if diffusion_start_indices.numel() > 0 and kwargs.get('refresh_negative', True):
                ...
                # update past key values - handle both old and new DynamicCache API
                past_kv = negative_model_kwargs['past_key_values']
                if hasattr(past_kv, 'key_cache'):
                    # Old API
                    key_caches = past_kv.key_cache
                    value_caches = past_kv.value_cache
                elif hasattr(past_kv, 'to_legacy_cache'):
                    # Convert to legacy format
                    legacy = past_kv.to_legacy_cache()
                    key_caches = [layer[0] for layer in legacy]
                    value_caches = [layer[1] for layer in legacy]
                else:
                    # Assume tuple format
                    key_caches = [layer[0] for layer in past_kv]
                    value_caches = [layer[1] for layer in past_kv]
